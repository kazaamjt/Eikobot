...
"""
The parser is the most important and largest piece of the Eikobot Compiler.
...
@dataclass
class UnionTypeExprAST(TypeExprAST):
    """An ExprAST expressing a union type."""

    def compile(self, context: CompilerContext) -> EikoType:
        name = ""
        compiled_types: list[EikoType] = []
        for subtype in self.sub_expressions:
            compiled_expr = subtype.compile(context)
            compiled_types.append(compiled_expr)
            name += compiled_expr.name + " |"

        name = name[:-2]

        return EikoUnion(name, compiled_types)
...
class Parser:
    ...
    def _parse_type(self, union_expressions: list[TypeExprAST] | None = None) -> TypeExprAST:
        ...
        if self._current.type == TokenType.PIPE:
            type_expr = TypeExprAST(primary_expr.token, primary_expr, sub_expressions)
            if union_expressions is None:
                union_expressions = [type_expr]
            else:
